...
- "booking": an object holding the details needed to write a contract and open a client portal. This is DATA, not prose: do not translate it, do not add commentary, do not write "unknown" or "TBD". Every key below MUST be present.
  ...
  If the same key is given more than one value over the thread, use the MOST RECENT. Two exceptions. "session_durations" is a list and keeps every statement it finds, oldest first. And A CHOICE IS NOT A REVISION: when ONE message offers alternatives ("A costs this, or B costs that", "Saturday or Sunday"), it has not changed its mind between them, it has put two options on the table. Resolve it by what was actually taken, not by which came last in the sentence. See "total_amount" for the worked case.
    ...
    - "total_amount": the full price named for THIS shoot, digits only, no currency symbol and no words: "500", not "$500 for 3 hours". A figure Vero quoted counts, full stop: acceptance, confirmation, a deposit and a signature are all irrelevant to this key.
      WHEN SEVERAL TOTALS APPEAR, work out which one is LIVE. Two different situations look identical if you only sort by position in the thread.
      (a) A REVISION. Vero named a price and later named a different one for the same thing. The later one wins.
      (b) A CONDITIONAL OFFER, which is one message naming two prices for two different options. Vero wrote: "since the drive to Malcolm Gross Rose Garden is only about an hour and 15 minutes, I can keep the price at $400 without additional travel costs. However, if you prefer a location that is about 2 hours away, the total would be $450." That is not a revision from 400 to 450. It is $400 for one location and $450 for another, and taking the last number in the paragraph would put a price on the contract for a place they are not going to.
      Resolve a conditional against what the rest of the thread has SETTLED. If "event_location" is the place the cheaper price was attached to, the total is the cheaper one. If the customer has picked the other option, it is the other one. If nothing in the thread chooses between them yet, use the one Vero put forward as her recommendation, and make the condition visible in "gathered" so she can see both.
      Do NOT use: an hourly or per-item rate that was never multiplied out into a total, one option from a list of packages the customer has not chosen between, a retainer or deposit, a travel or add-on fee on its own, or a number the customer floated as a budget that Vero never quoted. null only when no total for this shoot appears anywhere in the thread.
