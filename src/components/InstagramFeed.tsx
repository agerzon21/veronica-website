...
// Square tile derivatives for the nine local fallback photos, generated by
// scripts/build-ig-fallbacks.mjs. The grid renders 1:1 tiles at ~114 CSS px on
// mobile; the originals behind these are 4.99 MiB of full-resolution gallery
// files up to 3808px wide. srcSet lets the browser take ~74 KiB instead.
//
// Only the GRID uses these. photo.url stays the original because it is the
// React key and what IgPostModal renders in an 85vh lightbox, where a square
// centre-crop would be wrong.
const FALLBACK_SRCSET: Record<string, Record<string, string>> = igFallbackVariants;

const tileSrcSet = (url: string): string | undefined => {
  const rungs = FALLBACK_SRCSET[url];
