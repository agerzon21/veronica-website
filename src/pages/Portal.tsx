...
const Portal = () => {
  ...
  return (
    ...
    <Box position="relative" minH="100vh" overflow="hidden" bg="brand.surfaceSunken">
      ...
      <PortalMosaic veil={{ base: 0.55, md: 0.55 }} />
      ...
      <Flex
        ...
        pb={{ base: 16, md: 12 }}
      >
        {/* WIDER THAN THE FORM, and only on a wide screen.
            The header used to live inside the 460px form column, which is
            fine until the title does not fit in it: "A home for every story"
            is about 470px at the 3.25rem this page uses on a wide screen, so
            it wrapped to two lines by roughly nine pixels. That mattered more
            than it sounds. A soft spot is sized from the box it sits behind,
            and a wrapped h1's box stays the full column width however short
            its longest line is, so the spot came out 536px wide over 258px of
            balanced text: a slab, not a patch, and nothing like what was
            tuned. Giving the header room to keep the title on one line makes
            the box and the words the same width again.
            The doors below keep the 460px they always had. */}
        <Box w="100%" maxW={{ base: '460px', md: '720px' }}>
          ...
          <Reveal immediate from={{ opacity: 0 }} duration={0.6}>
            <VStack spacing={8}>
              ...
              <VStack spacing={3} w="100%" maxW="460px" mx="auto">
                <PortalDoor
 ...
 */
const SPOT = {
  // ALEX'S OWN NUMBERS, read straight off the Tweaks panel in the screenshot
  // he sent, and applied at both widths because the desktop board is the one
  // he tuned and "exactly like that" is what he asked for.
  //
  // softness 0 everywhere is deliberate on his part: no falloff, a hard edge.
  // It is not an oversight to be helpfully smoothed away.
  eyebrow: {
    a: { base: 1, md: 1 },
    x: { base: '5px', md: '5px' }, y: { base: '4px', md: '4px' },
    soft: { base: 0, md: 0 },
  },
  rule: {
    a: { base: 1, md: 1 },
    x: { base: '5px', md: '5px' }, y: { base: '4.5px', md: '4.5px' },
    soft: { base: 0, md: 0 },
  },
  title: {
    a: { base: 0.78, md: 0.78 },
    x: { base: '38px', md: '38px' }, y: { base: '4.5px', md: '4.5px' },
    soft: { base: 0, md: 0 },
  },
  offramp: {
    a: { base: 1, md: 1 },
    x: { base: '17px', md: '17px' }, y: { base: '6px', md: '6px' },
    soft: { base: 0, md: 0 },
  },
} as const;
...
// roundness 25, his setting.
const SPOT_RADIUS = { base: roundness(25), md: roundness(25) };
