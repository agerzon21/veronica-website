import { Box, VStack, Text, Flex, Button, Icon } from '@chakra-ui/react';
import { Helmet } from 'react-helmet-async';
import { motion, useInView } from 'framer-motion';
import { useRef } from 'react';
import { useCopyNotification } from '../components/CopyNotification';
import { FaHeart } from 'react-icons/fa';

const MotionDiv = motion.div;

const ZELLE_PHONE = '(570) 909-5707';

// Zelle wordmark logo (official purple, from Wikimedia Commons — public domain)
const ZelleLogo = ({ width = 120 }: { width?: number }) => (
  <svg width={width} height={width * 0.527} viewBox="0 0 1200 633" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">
    <rect height="100%" width="100%" fill="#6c1cd3" rx="80" />
    <g transform="translate(0,633) scale(0.1,-0.1)" fill="white" stroke="none">
      <path d="m 3030,4179.9975 -657.2625,6.0526 -7e-4,-436.0501 H 3462 L 2330.0025,2299 V 1929.9975 H 3030 V 1505 h 420 v 425 h 714 c 6,17 8.1053,62.8446 -29.8947,104.8446 -63,70 -113.1053,143.1579 -159.1053,235.1579 -21,41 -36.9474,90.1053 -42.9474,94.1053 C 3791.448,2370.55 2982.5639,2368.8831 2983,2374 l 1133,1459.8947 v 346.1028 H 3450 V 4610 h -420 z" />
      <path d="M6063 4383 c-10 -4 -13 -225 -13 -1062 0 -1131 -1 -1112 49 -1209 45 -87 155 -165 278 -197 68 -18 268 -20 345 -4 99 21 118 51 79 128 -25 48 -58 153 -66 211 -11 69 -25 80 -109 78 -40 -1 -78 2 -85 6 -41 26 -41 33 -41 1046 0 651 -3 978 -10 991 -10 18 -23 19 -213 18 -111 0 -208 -3 -214 -6z" />
      <path d="M6980 4380 c-13 -8 -15 -136 -15 -1052 0 -987 1 -1047 19 -1116 34 -136 111 -219 257 -280 56 -23 81 -27 194 -30 151 -4 228 7 305 46 76 37 78 63 13 158 -26 38 -66 103 -88 144 l-40 75 -73 1 c-91 2 -100 6 -117 49 -13 29 -15 178 -15 1005 0 651 -3 978 -10 991 -10 18 -23 19 -213 19 -111 0 -209 -5 -217 -10z" />
      <path d="M9640 4165 c-81 -37 -123 -101 -124 -190 -1 -76 30 -134 93 -175 78 -53 203 -38 268 32 83 89 66 243 -36 312 -53 36 -146 45 -201 21z m160 -37 c113 -57 130 -203 34 -288 -41 -36 -70 -44 -133 -38 -174 17 -208 261 -46 334 44 20 95 17 145 -8z" />
      <path d="M9647 4083 c-7 -12 -8 -198 -1 -217 3 -9 12 -16 20 -16 10 0 14 14 14 50 0 45 2 50 23 50 16 0 30 -14 52 -50 18 -30 36 -50 47 -50 24 0 23 4 -14 58 -22 32 -28 47 -19 50 26 9 51 42 51 67 0 45 -32 65 -104 65 -36 0 -67 -3 -69 -7z m121 -32 c8 -4 12 -19 10 -32 -3 -21 -9 -24 -50 -27 l-48 -3 0 35 c0 35 1 36 38 36 20 0 43 -4 50 -9z" />
      <path d="M4805 3755 c-5 -2 -44 -11 -85 -20 -257 -59 -484 -256 -580 -506 -53 -138 -72 -245 -73 -404 -1 -223 44 -387 147 -543 116 -175 286 -294 506 -354 75 -20 108 -23 280 -23 170 0 205 3 276 22 176 49 326 130 447 245 77 72 76 92 -2 152 -32 25 -90 79 -128 121 -48 51 -77 75 -92 75 -14 0 -46 -20 -77 -50 -107 -97 -242 -143 -419 -142 -162 1 -280 43 -355 129 -38 43 -86 141 -96 196 l-6 37 623 0 c343 0 630 4 637 9 33 21 41 166 17 324 -57 378 -283 632 -638 717 -63 15 -115 20 -227 19 -80 0 -149 -2 -155 -4z m267 -386 c73 -15 163 -65 205 -113 39 -44 79 -123 89 -174 l7 -32 -408 0 -407 0 7 32 c30 135 146 248 290 284 64 16 150 17 217 3z" />
      <path d="m 8440,3746 c -355,-79 -595,-329 -670,-698 -45,-218 -20,-472 65,-654 116,-252 327,-415 615,-474 91,-19 127,-21 270,-17 121,4 186,11 242,26 171,45 321,127 436,237 88,85 88,89 -7,168 -44,36 -103,93 -131,126 -65,75 -82,76 -155,10 -109,-99 -239,-143 -420,-142 -254,1 -402,110 -450,330 l -7,25.95 h 1267 c 43,31 27,359.05 -25,511.05 -10,30 -37,90 -59,132 -117,224 -304,363 -564,418 -83,18 -328,18 -407,1 z m 312,-377 c 68,-15 138,-50 188,-95 42,-39 110,-162 110,-200 v -24 h -812 l 7,28 c 52,214 272,341 507,291 z" />
    </g>
  </svg>
);

const Pay = () => {
  const contentRef = useRef<HTMLDivElement>(null);
  const isInView = useInView(contentRef, { once: true, amount: 0.15 });
  const { show: showCopied, Notification: CopyNotification } = useCopyNotification('Number Copied');

  const handleCopy = () => {
    navigator.clipboard.writeText('5709095707').then(() => showCopied());
  };

  return (
    <Box h="100vh" overflow="hidden" bg="white">
      <CopyNotification />
      <Helmet>
        <meta name="robots" content="noindex, nofollow" />
        <title>Pay - Vero Photography</title>
      </Helmet>

      <Flex
        justify="center"
        px={6}
        pt={{ base: 32, md: 28 }}
        pb={10}
      >
        <Box ref={contentRef} w="100%" maxW="420px">
          <MotionDiv
            initial={{ opacity: 0, y: 20 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.8, ease: "easeOut" }}
          >
            <VStack spacing={0}>
              {/* Top section */}
              <Box
                w="100%"
                bg="gray.50"
                borderTopRadius="lg"
                border="1px solid"
                borderColor="gray.200"
                borderBottom="none"
                px={8}
                py={6}
                textAlign="center"
                position="relative"
                overflow="hidden"
              >
                {/* Watermark heart */}
                <Icon
                  as={FaHeart}
                  color="#c9a96e"
                  opacity={0.06}
                  boxSize={{ base: '90px', md: '110px' }}
                  position="absolute"
                  top="50%"
                  left="50%"
                  transform="translate(-50%, -50%)"
                  pointerEvents="none"
                />
                <VStack spacing={2} position="relative">
                  <Text
                    fontSize={{ base: 'xl', md: '2xl' }}
                    fontWeight="200"
                    color="gray.800"
                    textTransform="uppercase"
                    letterSpacing="0.25em"
                  >
                    Thank You!
                  </Text>
                  <Text
                    fontSize="sm"
                    color="gray.500"
                    fontWeight="300"
                  >
                    Your support means the world to me.
                  </Text>
                </VStack>
              </Box>

              {/* Zelle details */}
              <Box
                w="100%"
                bg="white"
                borderBottomRadius="lg"
                border="1px solid"
                borderColor="gray.200"
                px={8}
                py={10}
              >
                <VStack spacing={6}>
                  <VStack spacing={3}>
                    <ZelleLogo width={140} />
                    <Text
                      fontSize="xs"
                      color="gray.400"
                      fontWeight="300"
                      letterSpacing="0.15em"
                      textTransform="uppercase"
                    >
                      Send via Zelle
                    </Text>
                  </VStack>

                  <Box w="100%" h="1px" bg="gray.100" />

                  <VStack spacing={1}>
                    <Text
                      fontSize="xs"
                      color="gray.400"
                      fontWeight="300"
                      letterSpacing="0.1em"
                      textTransform="uppercase"
                    >
                      Phone Number
                    </Text>
                    <Text
                      fontSize={{ base: '2xl', md: '3xl' }}
                      fontWeight="200"
                      color="gray.800"
                      letterSpacing="0.05em"
                    >
                      {ZELLE_PHONE}
                    </Text>
                  </VStack>

                  <Button
                    onClick={handleCopy}
                    bg="#c9a96e"
                    color="white"
                    fontSize="xs"
                    fontWeight="400"
                    letterSpacing="0.15em"
                    textTransform="uppercase"
                    h="44px"
                    w="100%"
                    borderRadius="sm"
                    _hover={{ bg: '#d4b87a', transform: 'translateY(-1px)' }}
                    _active={{ bg: '#b8964f', transform: 'translateY(0)' }}
                    transition="all 0.3s"
                  >
                    Copy Number
                  </Button>

                  <Text
                    fontSize="xs"
                    color="gray.400"
                    fontWeight="300"
                    letterSpacing="0.03em"
                    textAlign="center"
                    lineHeight="1.8"
                  >
                    Open your banking app, select Zelle,
                    <br />
                    and send to the number above.
                  </Text>
                </VStack>
              </Box>
            </VStack>
          </MotionDiv>
        </Box>
      </Flex>
    </Box>
  );
};

export default Pay;
